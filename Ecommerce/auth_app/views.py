...
def signup(request):
    if request.method=="POST":
        ...
        message = render_to_string('activate.html',{
            'user':user,
            'domain':'127.0.0.1:8000',   # You can change domain
        ...
        email_message.send()
        messages.success(request,"Activate Your Account by clicking the Link in your Gmail Inbox Or Spam")
...
class ActivateAccountView(View):
    def get(self,request,uidb64,token):
        ...
        if user is not None and generate_token.check_token(user,token):
            user.is_active=True
            user.save()
            messages.info(request,"Your Account is Activated Successfully")
            return redirect('/auth/login')
